...
static Function CONSAPI()
	...
	Local cBaseUrl := "urlbase" //exemplo
	Local cPath    := 'api/path' //exemplo
	Local cRet     := '{ "itens": '
	Local dDataIni
	Local dDataFim
	Local cParams
	...
	cParams  := "params" //exemplo
	cAuth    := GetMV("MV_TOKEN")
	...
	If !SF2->(DbSeek(aCabSF2[9][2] + aCabSF2[7][2])) //verifica se já há um registro com essas informações
		...
		Begin Transaction
			...
			If lMsErroAuto
				...
				ZA3->(DbSetOrder(1)) //Posiciona no indice 1
				ZA3->(DbGoTop())
